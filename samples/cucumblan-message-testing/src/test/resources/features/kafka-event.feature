...
Scenario: check produce and consume event validation 1
    Given send inline message event TEST on pet with type JSON
    ...
    When verify-by-elements TEST contains 200 on pet with type JSON
        | id   | i~200|
    Then verify TEST contains 200 on pet with type JSON
...
Scenario: check produce and consume event validation 2
    ...
    When verify-by-elements TEST contains 101 on pet with type JSON
        | id            | i~101             |
        | category.name | german shepherd   |
    Then verify TEST contains 101 on pet with type JSON
        |id,name, category/id:name,status|
        |i~101,Rocky,i~100:german shepherd,available|
    And verify TEST contains 101 on pet with type JSON
        |id,name, category/id:name,tags/id:name,status,photoUrls|
...
Scenario: check produce and consume event validation 3
    ...
    When verify-by-elements TEST contains 102 on pet with type JSON
        | id            | i~102             |
        | category.name | german shepherd   |
    Then verify TEST contains 102 on pet with type JSON
        |id,name, category/id:name,status|
        |i~102,Rocky,i~100:german shepherd,available|
    And verify TEST contains 102 on pet with type JSON
        |id,name, category/id:name,tags/id:name,status,photoUrls|
