...
public class StepDefinitionHelper {
	...
	public static String getActualResource(Properties props, String resourceKey) {
		return props.getProperty("BASE_URL", "http://localhost/")
				+ (props.getProperty(resourceKey) != null ? props.getProperty(resourceKey) : resourceKey);
	}

	public static String buildInputRequest(Map<String, String> parameterMap)
			throws Exception {
		String inputFileName = parameterMap.get("inputJson");
		try {
			StringBuffer sb = new StringBuffer();
			BufferedReader br = new BufferedReader(
					new InputStreamReader(StepDefinitionHelper.class.getResourceAsStream(inputFileName), "UTF-8"));
			...
			e.printStackTrace();
			throw new Exception(
					"Validate " + inputFileName + " is missing or has some issue(s) with " + e.getMessage());
		}
	}
